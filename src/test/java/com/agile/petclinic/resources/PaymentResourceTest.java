...
class PaymentResourceTest {
	...
	@BeforeAll
	static void initAll() {
		ap1 = new Appointment(1L, Instant.parse("2021-05-13T12:00:00Z"), "Routine exam",
				AppointmentType.MEDICAL, null, null, null);
		ap2 = new Appointment(2L, Instant.parse("2021-05-14T11:00:00Z"), "Surgery",
				AppointmentType.MEDICAL, null, null, null);
	...
	void postMethod() throws Exception {		
		...
		this.mockMvc.perform(post("/payments").contentType("application/json").content(Obj.writeValueAsString(objDTO))).andDo(print()).andExpect(status().isCreated())
		.andExpect(content().json(Obj.writeValueAsString(pay3)));
	}
	...
	void putMethod() throws Exception {		
		...
		this.mockMvc.perform(put("/payments/" + pay3.getId()).contentType("application/json").content(Obj.writeValueAsString(pay3))).andDo(print()).andExpect(status().isOk())
		.andExpect(content().json(Obj.writeValueAsString(pay3)));
	}
}
